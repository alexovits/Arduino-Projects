  ...
  while ((!string_complete)&&(char_position<max_password_length))
    {
      ...
      while (!char_read)
      {
          ...
          if (char_read=='#') string_complete = true;
    }
  ...
  if (!password_was_set){
    ...
    password_request:
      ...
      if(strcmp(password,repeated_password) == 0){
        Serial.print("The password is set");
        Serial.println();
      }else{
        Serial.print("The passwords do not match. Try again.");
        Serial.println();
        // If passwords do not match jump back to reading them again
